...
def loss_l2(D_logits_real, D_logits_fake, D_logits_fake_for_G):
  G_loss = tf.reduce_mean(tf.nn.l2_loss(D_logits_fake_for_G - tf.zeros_like(D_logits_fake_for_G))) 

  D_loss_real = tf.reduce_mean(tf.nn.l2_loss(D_logits_real - tf.ones_like(D_logits_real))) 

  D_loss_fake = tf.reduce_mean(tf.nn.l2_loss(D_logits_fake + tf.ones_like(D_logits_fake))) 
...
def train(G_loss, D_loss, G_vars, D_vars, global_step):
  ...
  G_optim = tf.train.RMSPropOptimizer(FLAGS.learning_rate) 
  D_optim = tf.train.RMSPropOptimizer(FLAGS.learning_rate) 
